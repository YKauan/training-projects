...
export const filterTrigger = trigger('filterAnimation', [
  transition(':enter', [
    ...
    animate('400ms ease-out', keyframes([
      style({ offset: 0.0, opacity: 0,   width: 0   }),
      style({ offset: 0.8, opacity: 0.5, width: '*' }),
      style({ offset: 1,   opacity: 1,   width: '*' }),
    ]))
  ]),
  transition(':leave', [
    animate('400ms cubic-bezier(.17,.67,.96,.13)', style({
      opacity: 0,
      width: '0'
    }))
  ])
...
export const  formButtonTrigger = trigger('formButtonTrigger', [
  transition('invalid => valid',[
    query('#botao-salvar', [
      group([
        animate(200, style({
          backgroudColor: '#63B77C',
        })),
        animate(100, style({
          transform: 'scale(1.1)'
        })),
